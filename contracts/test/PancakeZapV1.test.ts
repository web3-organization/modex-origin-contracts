...
const QuantumCoinFactory = artifacts.require("./QuantumCoinFactory.sol");
const QuantumCoinPair = artifacts.require("./QuantumCoinPair.sol");
const QuantumCoinRouter = artifacts.require("./QuantumCoinRouter.sol");
const QuantumCoinZapV1 = artifacts.require("./QuantumCoinZapV1.sol");
const WBNB = artifacts.require("./WBNB.sol");

contract("QuantumCoinZapV1", ([alice, bob, carol, david, erin]) => {
  ...
  before(async () => {
    // Deploy Factory
    pancakeFactory = await QuantumCoinFactory.new(alice, { from: alice });

    // Deploy Wrapped BNB
    wrappedBNB = await WBNB.new({ from: alice });

    // Deploy Router
    pancakeRouter = await QuantumCoinRouter.new(pancakeFactory.address, wrappedBNB.address, { from: alice });

    // Deploy ZapV1
    maxZapReverseRatio = 100; // 1%
    pancakeZap = await QuantumCoinZapV1.new(wrappedBNB.address, pancakeRouter.address, maxZapReverseRatio, { from: alice });
  ...
  describe("Normal cases for liquidity provision and zap ins", async () => {
    it("User adds liquidity to LP tokens", async function () {
      ...
      /* Add liquidity (QuantumCoin Router)
       * address tokenB,
